...
function Profile() {
    ...
    const setFullName = (event: ChangeEvent | any, type: string) => {
        if (type === 'firstName') {
            ...
            setFirstNameValue((prevState) => {
                return {
                    ...prevState, middleName: event.target.value
                }
            });
    ...
    const submit = () => {
        if (firstNameValue) {
            dispatch(updateUsersDetails(firstNameValue));
        }
    }
    return (
            ...
            <div className={styles.profileDetails}>
                ...
                <Flex align={'center'} gap={6} mt={6} mb={6} maxWidth={'320px'}>
                    <div className={styles.profileAccount}>
                        ...
                        <Input placeholder='Enter First Name' value={firstNameValue.firstName}
                               onChange={(event) => setFullName(event, 'firstName')}/>

                        <Text fontSize={'14px'} mt={2}>Middle Name</Text>
                        <Input placeholder='Enter Middle Name' value={firstNameValue.middleName}
                               onChange={(event) => setFullName(event, 'middleName')}/>

                        <Text fontSize={'14px'} mt={2}>Last Name</Text>
                        <Input placeholder='Enter Last Name' value={firstNameValue.lastName}
                               onChange={(event) => setFullName(event, 'lastName')}/>
                    </div>
                </Flex>
            </div>
            <Flex align={'center'} className={styles.changeProfileButton} gap={5}>
                <Button height={'auto'} padding={'0'} className={styles.changePassword} variant='ghost'> Change
                    Password </Button>
                <Button height={'auto'} padding={'0'} className={styles.deleteProfile} variant='ghost'> Delete
                    Profile </Button>
            </Flex>
