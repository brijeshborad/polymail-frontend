    ...
    return (
        <>
            <Flex padding={'16px 40px'} backgroundColor={'#FFFFFF'} borderBottom={'1px solid #F3F4F6'} gap={3}
                  overflowX={'auto'}>
                {projectData && !!projectData.length && (projectData || []).map((item: Project, index: number) => (
                    <Flex align={'center'} key={index} gap={2} backgroundColor={'#FFFFFF'} border={'1px solid #F3F4F6'}
                        ...
                        <Text whiteSpace={'nowrap'} overflow={'hidden'} textOverflow={'ellipsis'} fontSize='13px'
                              color={'#0A101D'} flex={'1'}>{item.name}</Text>
