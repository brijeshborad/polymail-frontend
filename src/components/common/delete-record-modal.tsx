...
import {DeleteRecordModelType} from "@/types/props-types/delete-record-model.type";

function RemoveRecordModal(props: DeleteRecordModelType) {
    return (
        <Modal isOpen={props.isOpen ?? false} onClose={props?.onClose ?? (() => {})} isCentered>
            ...
            <ModalContent className={'confirm-modal'} borderRadius={12} boxShadow={'0 0 12px 0 rgba(0,0,0, 0.08)'} padding={'12px'} maxW={'420px'}>
                <ModalBody padding={'12px 12px 24px'}>
                    <Heading as='h5' fontSize={'15px'} color={'#0A101D'} lineHeight={1.21}>{props?.modelTitle ?? ''}</Heading>
                ...
                <ModalFooter className={'confirm-modal-footer'} borderTop={'1px solid #F3F4F6'} px={0} pb={0}>
                    ...
                    <Button className={'remove-button footer-button'} variant='ghost' onClick={() => (props.confirmDelete ? props.confirmDelete('delete') : (() => {}))}>Remove</Button>
                </ModalFooter>
            </ModalContent>
        </Modal>
    )
